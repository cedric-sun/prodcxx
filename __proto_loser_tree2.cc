...
// problem with storing T directly in the tree
// 1. when T is some heavy structure, need to copy assign (expensive) or
//      use move semantics (complex code);
// 2. cannot have marker value; every T is possibly valid input;
//      for int inputs, surely we can use long to make it work, but we will
//      be writing very specific code.
...
#define RIGHT false
    ...
    loser_tree(const vector<vector<int>> &t_lists)
        ...
        // during the init, tree[0] is undefined;
//        for (int i = 0; i < n; ++i) {
//            upfloat(i);//todo: is it better to top-bottom init and bottom-top replay?
//        }
        tree[0] = init(1);
        // tree[0] is the champion now;
    }
...
//            if (j == -1) { // uninitialized
//                tree[parent] = i;
//                break;
//            }
            //-----------------
            // compare my current head with the loser in my parent
